...
class Crawler:
    urls = {
        'years-page':'https://www4.planalto.gov.br/legislacao/portal-legis/legislacao-1/medidas-provisorias',
    }
    ...
    def parse_link(self, data_rows:list, progress_bar, year:int)->None:
        ...
        for data_row in data_rows:
            ...
            if len(datas)> 1:
                ...
                data_dou = None
            data_lei = f"{century}{str(data_lei.year)[2:]}-{data_lei.month:02d}-{data_lei.day:02d}"
            data_lei = datetime.strptime(data_lei, "%Y-%m-%d").date()
            _dict = {
                'num_lei': num_lei,
                'data_lei': data_lei,
                'data_dou': data_dou,
            ...
            for paragraph_tag in lei_soup.find_all('p'):
                ...
                optional_span =  paragraph_tag.find('span')
                optional_striked = ''
                if optional_span:
                    optional_striked = optional_span.get('style', '')
                if paragraph_tag.find('strike') or 'text-decoration:line-through' in paragraph_tag.get("style", '') or 'text-decoration:line-through' in optional_striked:
    ...
    def parse_link_with_workers(self, data_rows:list, year:str, workers:int = 1):
    ...
    def parse_year_pages(self, pages:dict, workers:int=10):
        self.data = []
        pages_to_parse = {k: v for k, v in pages.items()}
        ...
        def run_parse_link(year, url, is_first_time:bool = True):
            ...
            if table:
                ...
                data_rows = rows[1:]
                self.parse_link_with_workers(data_rows, key, workers)
            else:
