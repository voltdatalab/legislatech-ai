...
class Crawler:
    urls = {
        'law-page':'https://www4.planalto.gov.br/legislacao/portal-legis/legislacao-historica/constituicoes-anteriores-1',
    }
    ...
    def parse_link(self, data_rows:list, progress_bar)->None:
        ...
        for data_row in data_rows:
            ...
            if not title:
                raise
            data_lei = re.search(r'\d+', title)
            if not data_lei:
                raise
            data_lei = data_lei.group(0)
            if not link:
            ...
            num_lei = None
            data_dou = None
            parsed_string = f"Consitutição de {data_lei}"
            ementa = None
