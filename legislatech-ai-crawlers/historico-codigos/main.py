...
class Crawler:
    urls = {
        'law-page':'https://www4.planalto.gov.br/legislacao/portal-legis/legislacao-1/codigos-1',
    }
    ...
    def parse_link(self, data_rows:list, progress_bar)->None:
        ...
        for data_row in data_rows:
            ...
            pattern = r'(?:LEI|DECRETO|DECRETO-LEI)\s*(?:N\s*[ºOo]|Nº|NO|no|N o|N°|n°|nº|Nº)\s*(\d{1,3}(?:\.\d{3})*)\s*,?\s*DE\s*(\d{1,2}(?:º)?)\s*DE\s*([a-zA-ZÀ-ú]+)\s*DE\s*(\d{4})'

            match = re.search(pattern, lei_soup.get_text(), re.IGNORECASE)
            if match:
                num_lei = match.group(1).replace('.', '')
                if not link.endswith(f"{num_lei}.htm"):
                    # por algum motivo ele tem esse outro aqui que vai totalmente contra o padrão sei la maluquice mas cai no match anterior
                    #tem jeitos melhores mas vida que segue
                    pattern = r'EXPOSIÇÃO\s+DE\s+MOTIVOS\s+Nº\s+(\d{1,3}),\s+DE\s+(\d{1,2})\.(\d{1,2})\.(\d{4})'
                    match = re.search(pattern, lei_soup.get_text(), re.IGNORECASE)
                    if match:
                        # podia ter só pegado e dado replace no . por /, mas to maluco ja
                        num_lei = match.group(1).replace('.', '')
                        dia = match.group(2)
                        mes = match.group(3)
                        ano = match.group(4)
                        ...
                        print(f"link: {link} com num_lei {num_lei} e data_lei vazias")
                        input()
                else:
                    day = match.group(2).replace("º", "").zfill(2)
            ...
            print(f"Número da Lei: {num_lei}")
            print(f"Data da Lei: {data_lei}")
