...
class Crawler:
    urls = {
        'years-page':'https://www4.planalto.gov.br/legislacao/portal-legis/legislacao-1/leis-ordinarias',
    }
    ...
    def _initialize_session_with_playwright(self):
        ...
        url_to_visit = self.urls['years-page']
        with sync_playwright() as p:
            ...
            try:
                ...
                page.wait_for_selector('#content-core', timeout=30000)
    ...
    def get_pages(self):
        response = self._do_get_page(self.urls['years-page'])
        soup = BeautifulSoup(response.text, 'html.parser')
        link_div = soup.find('div',{'id':'content-core'})
        par_tags = link_div.find_all('p')
        links = {}

        #o ano atual n fica na lista, fica separado no unico <h2> da page

        current_year_obj = soup.find('h2')
        if not current_year_obj:
            raise ValueError("Não foi possível encontrar o ano atual na página")
        current_year = current_year_obj.text.strip()
        link = current_year_obj.find("a").get('href')
        links[current_year] = link

        for par_tag in par_tags:
            link_tags = par_tag.find_all('a')
            for link_tag in link_tags:
                print(link_tag)
                links[link_tag.text.strip()] = link_tag.get('href')
        return links

    def parse_link(self, data_rows:list, progress_bar, year:int)->None:
        for data_row in data_rows:
            ...
            if 'swedenberger' in names:
                _dict['other_sancionadores'] += ",Swedenberger do Nascimento Barbosa"
            if 'macae' in names:
                _dict['other_sancionadores'] += ",Macaé Maria Evaristo dos Santos"
    ...
    def parse_link_with_workers(self, data_rows:list, year:int, workers:int = 10):
        ...
        with tqdm(total=len(data_rows), desc="Parsing links") as progress_bar:
            for i in range(workers):
                ...
                thread = threading.Thread(target=self.parse_link, args=(data_rows[start:end], progress_bar, year))
    ...
    def parse_year_pages(self, pages:dict, workers:int=10):
        self.data = []
        pages_to_parse = {k: v for k, v in pages.items() if k.isdigit()}


        def run_parse_link(year, url, is_first_time:bool = True):
            year_progress_bar.set_description(f"Parsing year page: {key}")
            ...
            if table:
                ...
                data_rows = rows[1:]
                self.parse_link_with_workers(data_rows, int(key), workers)
            else:
                # se caiu aqui provavelmente é pq tomou bloqueio, só reiniciar a sessão e tentar novamente
                print(f"Tabela de leis não encontrada na página do ano {key}")
                if is_first_time:
                    self._initialize_session_with_playwright()
                    return run_parse_link(year, url, False)

            year_progress_bar.update(1)

        with tqdm(total=len(pages_to_parse), desc="Parsing year pages", unit="year") as year_progress_bar:
            for key, value in pages_to_parse.items():
                run_parse_link(key, value)
...
if __name__ == "__main__":
    ...
    laws = c.parse_year_pages(x, workers)
    df = pd.DataFrame(laws)
    df.to_csv('leis_ordinarias.csv', index=False, sep=';', encoding='utf-8-sig')
